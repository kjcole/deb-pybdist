...
def _move_top_files_to_dir(from_dir, to_dir):
  ...
  print(f'Moved files to {to_dir!r}')
...
def _run_or_die(args, output=True):
  ...
  if ret:
    raise DebianException(f'Error running: {" ".join(args)!r}')
...
def _get_deb_dir():
  ...
  if os.path.exists('/etc/debian_version'):
    ...
    deb_ver = 'UNKNOWN'
  return f'debian-{deb_ver}'

def build_deb(setup):
  tmpdir = 'tmp'
  shutil.rmtree(tmpdir, True)
  dest_dir = f'{setup.NAME}-{setup.VER}'
  src_tar = f'{setup.NAME}-{setup.VER}'
  dest_tar = f'{setup.NAME}_{setup.VER}'
  _copy_dir('debian', os.path.join(tmpdir, dest_dir, 'debian'))

  src_tarname = src_tar + '.tar.gz'
  dest_tarname = dest_tar + '.tar.gz'
  print(f'Linking dist/{src_tarname!r} to tmp/{dest_tarname!r}')
...
def control_file(setup):
  ...
  lines.append(f'Source: {setup.NAME}')
  lines.append('Section: python')
  lines.append('Priority: optional')
  lines.append(f'Maintainer: {setup.SETUP["author"]} <{setup.SETUP["author_email"]}>')
  lines.append('Build-Depends: cdbs, debhelper (>= 7), python (>= 2.4), python-support')
  lines.append('Standards-Version: 3.8.4')
  lines.append(f'Homepage: {setup.SETUP["url"]}')
  vcs = setup.VCS
  if vcs.endswith('/hg/'):
    lines.append(f'Vcs-Hg: {vcs}')
    lines.append(f'Vcs-Browser: {vcs}')
  lines.append('')
  lines.append(f'Package: {setup.DEB_NAME}')
  lines.append('Architecture: all')
  lines.append('Depends: ${shlibs:Depends}, ${misc:Depends},${python:Depends},')
  lines.append(f' {",".join(setup.DEPENDS)}')
  lines.append(f'Description: {setup.SETUP["description"]}')
  ...
  for line in long_desc:
    if line.strip():
      ...
      lines.append(' .')
  lines.append(f'# -- generated by {util.MAGIC_NAME}')
  return lines
...
def git_import_orig(setup):
  ...
  args = ['git-import-orig', '--pristine-tar',
    f'../../{_get_deb_dir()}/{setup.NAME}_{setup.VER}.orig.tar.gz',
    ]
  _run_or_die(args)
